...
var (
	...
	// ErrCreatedAtZero: the zero instant is what an unset field reads as — year 1 in
	// Go, 1970 wherever seconds are counted from the epoch — so a claim dated at
	// either carries a default where `V-MONO` requires the time it was added.
	ErrCreatedAtZero     = errors.New("ranke: created_at is the zero instant, which states no time a claim was added (`V-MONO`)")
