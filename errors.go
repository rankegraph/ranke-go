...
var (
	...
	// ErrCreatedAtPredatesRanke: no archive predates the foundation paper, so no claim
	// was added before it. An earlier created_at is what an unset field reads as — year
	// 1 in Go, 1970 where a clock never started — where `V-MONO` requires a time.
	ErrCreatedAtPredatesRanke = errors.New("ranke: created_at predates 2026-05-03, the day the design it conforms to was founded, so it states no time a claim was added (`V-MONO`)")
